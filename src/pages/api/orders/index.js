...
import GenerateOrderConfirmTemplate from "@/src/backend/helpers/templates/orderConfirm";
    ...
    switch(req.method) {
        ...
        case "POST":
            try {
                ...
                const orderDetail = 
                  await OrdersModel.findById(order._id)
                    .populate({ path: "subOrders.vendor_id", model: UsersModel, select: 'fullName address userType' })
                    .populate({ path: "user", model: UsersModel, select: 'fullName address userType phone' })
                    .populate({ path: "subOrders.carts.product_id", model: ProductsModel })
                  
                  const adminEmailTemplte = GenerateOrderConfirmTemplate(orderDetail);
                ...
                const userEmailTemplate = GenerateOrderConfirmTemplate(orderDetail);
                await EmailSend(
                  user.email.value,
                  user.fullName,
